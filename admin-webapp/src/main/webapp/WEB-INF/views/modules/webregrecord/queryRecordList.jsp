...
<body>
	...
	<div>
	<form name="form" role="form" action="${ctx}/webregrecord/queryRecordlist" method="GET">
	...
	取消状态：<select class="mSelect"  class="mSelect"  id="unsubscribeType" name="unsubscribeType">
		...
		<option value="5">医生调离</option>
		<option value="6">客服取消</option>
	</select>
	挂号类型：<select class="mSelect"  class="mSelect"  id="registerType" name="registerType">
	    <option value="">请选择</option>
		<option value="1">人工挂号</option>
		<option value="4">VIP挂号</option>
		<option value="5">现场挂号</option>
		<option value="7">行业预约</option>
		<option value="8">网站预约</option>
	</select>
    <input type="submit" id="searchDoc" value="查询"/>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <font color="#FF0000">*必须选择挂号类型* </font>
    <c:if test="${registerType=='7'}">
	...
	<div class="panel panel-default">
		...
		<table class="table table-bordered table-hover tablesorter table2">
			...
			<tr>
				...
				<c:if test="${registerType!='4'}">
				<td>
					<c:if test="${regRecord.unsubscribeType=='1'}">
	...
	<page:page pager="${page}" linkUrl="${ctx}/webregrecord/queryRecordlist?hospId=${params.hospId}&queryDiagnosisDateBegin=${params.queryDiagnosisDateBegin}&queryDiagnosisDateEnd=${params.queryDiagnosisDateEnd}&areaId=${params.areaId}&departmentId=${params.departmentId}&departmentName=${params.departmentName}&areaIdParent=${params.areaIdParent}&areaIdChild=${params.areaIdChild}&hospital=${params.hospital}&hdParent=${params.hdParent}&hdChild1=${params.hdChild1}&doctorId=${params.doctorId}&projectNum=${params.projectNum}&ikangCardNum=${params.ikangCardNum}&queryRegistrationDateBegin=${params.queryRegistrationDateBegin}&queryRegistrationDateEnd=${params.queryRegistrationDateEnd}&patientName=${params.patientName}&operatorName=${params.operatorName}&isGuide=${params.isGuide}&serviceType=${params.serviceType}&unsubscribeType=${params.unsubscribeType}&registerType=${params.registerType}"/>	
...
$(document).ready(function() {
	...
	if(registerType!=""){
		...
		$("#registerType option[value='"+registerType+"']").attr("selected", "selected");
		//$(".registerType").find("option[text='pxx']").attr("selected",true);
	}
...
function cancel(){
	...
	var now = new Date();
	latestUnsubscribeTime = latestUnsubscribeTime.replace("-","/");//替换字符，变成标准格式    
