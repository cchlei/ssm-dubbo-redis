...
<body>
	...
	<div>
	<form name="form" role="form" action="${ctx}/webregrecord/recordlist" method="GET">
    ...
    <input id="creatorId" name="creatorId" type="hidden" value="${creatorId}"/>
    <input id="creatorName" name="creatorName" type="hidden" value="${creatorName}"/>
    ...
    <input type="hidden" id="registerType" name="registerType" value="${registerType}"/>
	 ...
	 <c:if test="${belongHospId==null || belongHospId==''}">
	    ...
	    <select class="mSelect"  class="mSelect"  id="areaIdChild" name="areaIdChild" onchange="searchHosp(null,this.value);">
	    <c:choose>
	...
	<page:page pager="${page}" linkUrl="${ctx}/webregrecord/recordlist?hospId=${params.hospId}&queryDiagnosisDateBegin=${params.queryDiagnosisDateBegin}&queryDiagnosisDateEnd=${params.queryDiagnosisDateEnd}&areaId=${params.areaId}&departmentId=${params.departmentId}&departmentName=${params.departmentName}&vip=${params.vip}&scene=${params.scene}&areaIdParent=${params.areaIdParent}&areaIdChild=${params.areaIdChild}&hospital=${params.hospital}&hdParent=${params.hdParent}&hdChild1=${params.hdChild1}&doctorId=${params.doctorId}&projectNum=${params.projectNum}&ikangCardNum=${params.ikangCardNum}&queryRegistrationDateBegin=${params.queryRegistrationDateBegin}&queryRegistrationDateEnd=${params.queryRegistrationDateEnd}&patientName=${params.patientName}&operatorName=${params.operatorName}&isGuide=${params.isGuide}&serviceType=${params.serviceType}&unsubscribeType=${params.unsubscribeType}&sortOrder=${params.sortOrder}&registerType=${params.registerType}&creatorName=${creatorName}&creatorId=${creatorId}&token=${token}&timestamp=${timestamp}"/>	
	...
	<div id="divSCA">
	   ...
	   &nbsp;&nbsp;&nbsp;&nbsp;请选择取消原因：<select id="cause" name="cause" class="lSelect">
	      <option value="3" selected="selected">${causeMAp['3']}</option>
	   </select>
	   <br/><br/><br/><br/>
...
$(document).ready(function() {
	...
	if(areaIdParent!="" && areaId==areaIdParent){
		...
		if(hospId!=""){
			searchHosp(areaIdParent,null,hospId);
			...
			searchHosp(areaIdParent,null,null);
		}
	}else if(areaIdParent!="" && areaId!=areaIdParent){
		searchArea(areaIdParent,areaId);
		if(hospId!=""){
			searchHosp(areaIdParent,areaId,hospId);
			searchDep(null,hospId,hdParent);
			if(hdParent!=null && hdParent!=departmentId){
				searchHd(null,1,hdParent,departmentId,null);
				$("#departmentId").val(departmentId);
			...
			searchHosp(areaIdParent,areaId,null);
		}
	}
});
	...
	$.post(url,{'recordId':registrationRecordId},function(json){
		if(json=='success'){
			$.alert('短信重新发送', function() {
				...
				if("${vip}"!=""){
					var vip = "${vip}";
					window.location.href = '${ctx}/webregrecord/recordlist?doctorId='+json.results+"&vip="+vip+"&flag=1"+"&creatorName="+creatorName+"&creatorId="+creatorId;
				}else if("${scene}"!=""){
					var scene = "${scene}";
					window.location.href = '${ctx}/webregrecord/recordlist?doctorId='+json.results+"&scene="+scene+"&flag=1"+"&creatorName="+creatorName+"&creatorId="+creatorId;
				}else{
					window.location.href = '${ctx}/webregrecord/recordlist?doctorId='+json.results+"&flag=1"+"&creatorName="+creatorName+"&creatorId="+creatorId;
				}
...
/* 		$.get(ctx+"/webregrecord/cancel/"+registrationRecordId,{'doctorId':doctorId,'cause':cause}, function(json) {
			var msg = "";
			if (json.message) {
				msg = json.message;
			}
			$.alert(msg, function() {
				...
				if("${vip}"!=""){
					var vip = "${vip}";
					window.location.href = '${ctx}/webregrecord/recordlist?doctorId='+json.results+"&vip="+vip+"&flag=1"+"&creatorName="+creatorName+"&creatorId="+creatorId+"&token="+token+"&timestamp="+timestamp;
				}else if("${scene}"!=""){
					var scene = "${scene}";
					//alert(json.results);
					window.location.href = '${ctx}/webregrecord/recordlist?doctorId='+json.results+"&scene="+scene+"&flag=1"+"&creatorName="+creatorName+"&creatorId="+creatorId+"&token="+token+"&timestamp="+timestamp;
				}else{
					window.location.href = '${ctx}/webregrecord/recordlist?doctorId='+json.results+"&flag=1"+"&creatorName="+creatorName+"&creatorId="+creatorId+"&token="+token+"&timestamp="+timestamp+"&callback=";
				}
				//window.location.reload();
			});
		}, "jsonp"); */
...
function searchArea(areaId,areaIdChild){
	...
	if(areaIdChild==null){
		searchHosp(areaId,null,null);//根据父级地区查询医院
	}
	
}

//医院下拉联动
function searchHosp(parentAreaId,areaId,hospId){
	if(areaId==0 && parentAreaId==0){
		return;
	}
	
	if(areaId==""||areaId==null ||  typeof(areaId)=="undefined"){
		areaId = parentAreaId;
	}
	$("#hospId").val("");
	var url = ctx+"/webregrecord/searchHosp";
	$.ajax({  
		type : "post",  
		url : url,  
		data : {'areaId':areaId},  
...
function searchDep(obj,hospId1,hdParent){
	...
	var url = ctx+"/webregrecord/searchDep";
...
function searchDoctor(departmentId,doctorId){
	var url = ctx+"/webregrecord/searchDoctor";
...
function download(){
	...
	var url="${ctx}/webregrecord/industryDownload?hospId="+hospId
