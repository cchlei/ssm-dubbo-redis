...
<body>
	...
	<div>
	<form name="form" role="form" action="${ctx}/regrecord/queryRecordlist" method="GET">
		...
		<table class="table table-bordered table-hover tablesorter table2">
			...
			<tr>
				...
				<td>
					...
					<button type="button" class="btn btn-primary btn-sm" onclick="toCancel('${regRecord.registrationRecordId}','<fmt:formatDate value="${regRecord.latestUnsubscribeTime}" pattern="yyyy-MM-dd"/>','${regRecord.doctorId}')">停诊</button>
					</c:if>
					<c:if test="${regRecord.unsubscribeType=='4'}">已停诊</c:if>
				</td>
				</c:if>
