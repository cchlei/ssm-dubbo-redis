...
<body>
	...
	<div>
	<form name="form" role="form" action="${ctx}/stopInfo/recordlist" method="GET">
		...
		<div class="panel-heading">
			<h4 class="panel-title">
				...
				<c:choose>
					...
					<c:otherwise>
						<button type="button" class="btn btn-primary btn-sm" id="regRecordIdCheckBoxCancel" style="float: right;margin-top: -6px;">多选停诊</button>
					</c:otherwise>
				</c:choose>
			</h4>
		</div>
		<table class="table table-bordered table-hover tablesorter table2">
			...
			<tr>
				...
				<td>
					...
					<button type="button" class="btn btn-primary btn-sm" onclick="stopInfo('${regRecord.registrationRecordId}','${regRecord.hospName}','${regRecord.departmentName}','${regRecord.doctorName}','${regRecord.patientName}','${regRecord.phoneNum}','${regRecord.diagnosisDate}')">停诊</button>
					</c:if>
 ...
 *停诊 
 */
function stopInfo(registrationRecordId,hospName,departmentName,doctorName,patientName,phoneNum,diagnosisDate){
	/* var registrationRecordId = $("#registrationRecordId").val();
	var name = $("#name").val();
	var phone = $("#phone").val();
	var doctorName = $("#doctorName").val();
	var treatmentTime = $("#treatmentTime").val();
	var department = $("#department").val();
	var hospitalName = $("#hospitalName").val();
	closeDiv(); */

	$.confirm("确定要停诊？确认后不可恢复！", function() {
		$.get(ctx+"/stopInfo/stop/"+registrationRecordId,{'name':patientName,'phone':phoneNum,'doctorName':doctorName,'treatmentTime':diagnosisDate,'department':departmentName,'hospitalName':hospName}, function(json) {
			var msg = "";
			if (json.message) {
				msg = json.message;
			}
			$.alert(msg, function() {
				if("${vip}"!=""){
					var vip = "${vip}";
					window.location.href = '${ctx}/stopInfo/recordlist?doctorId='+json.results+"&vip="+vip+"&flag=1";;
				}else if("${scene}"!=""){
					var scene = "${scene}";
					//alert(json.results);
					window.location.href = '${ctx}/stopInfo/recordlist?doctorId='+json.results+"&scene="+scene+"&flag=1";
				}else{
					window.location.href = '${ctx}/stopInfo/recordlist?doctorId='+json.results+"&flag=1";
				}
