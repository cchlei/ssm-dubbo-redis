...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="javascript:void(0)">医生缓存管理</a></li>
	</ul>
	<div>
	<form name="form" role="form" action="${ctx}/cache/list" method="GET">
		...
		<table class="table table-bordered table-hover tablesorter">
			...
			<tr>
			   ...
			   <%-- <c:if test="${doctor.isThirdparty=='2'}">  --%>
				...
				<a href="javascript:delDoctorCache('${doctor.hospId}','${doctor.departmentId}','${doctor.doctorId}')">清除缓存</a>
					</td>
				</td>
				</td>
				</c:if>
	...
	<page:page pager="${page}" linkUrl="${ctx}/doctor/list?areaId=${params.areaId}&areaIdParent=${params.areaIdParent}&flag=${params.flag}&hospId=${params.hospId}&departmentId=${params.departmentId}&departmentName=${params.departmentName}&standardDepartmentId=${params.standardDepartmentId}&hdParent=${params.hdParent}&isSpecialized=${params.isSpecialized}&doctorGrade=${params.doctorGrade}"/>
<script type="text/javascript">
var ctx = "${ctx}";
function delDoctorCache(hospId,departmentId,doctorId){
	$.confirm("确定要删除该医生缓存吗！", function() {
		$.get("${ctx}/cache/clearDoctorCache?hospId="+hospId+"&departmentId="+departmentId+"&doctorId="+doctorId, function(json) {
			var msg = "";
			if (json.message) {
				msg = json.message;
				alert(msg);
