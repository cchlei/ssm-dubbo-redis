	...
	<div class="panel panel-default">
		...
		<table class="table table-bordered table-hover tablesorter table2">
			...
			<tr>
				...
				<th class="text-center" colspan="16">详细信息</th>
				<th class="text-center" width="125px">操作</th>
			</tr>
			</thead>
			...
			<tr>
				<td class="td-center"><label id="writing-mode-tb-rl">${regRecord.patientName}</label></td>
				<td colspan="16">
				<div id="div-record">
					<label id="div-label-title">就诊人:</label><label id="div-label-content">${regRecord.patientName}</label>
					<c:if test="${regRecord.sex==1}">
						<label id="div-label-title">性别:</label><label id="div-label-content">男</label>
					</c:if>
					<c:if test="${regRecord.sex==2}">
						<label id="div-label-title">性别:</label><label id="div-label-content">女</label>
					</c:if>
					<label id="div-label-title">证件号:</label><label id="div-label-content">${regRecord.idCard}</label>
					<label id="div-label-title">电话:</label><label id="div-label-content">${regRecord.phoneNum}</label>
					<label id="div-label-title">社保卡:</label><label id="div-label-content">${regRecord.medicareNumber}</label>
				</div>	
				<div id="div-record">
					<label id="div-label-title">医院名称:</label><label id="div-label-content">${regRecord.hospName}</label>
					<label id="div-label-title">科室名称:</label><label id="div-label-content">${regRecord.departmentName}</label>
					<label id="div-label-title">医生:</label><label id="div-label-content">${regRecord.doctorName} </label>
					<label id="div-label-title">门诊类型:</label><label id="div-label-content">${regRecord.resourceTpyeText}</label>
				</div>
				<div id="div-record">
					<label id="div-label-title">就诊时间:</label><label id="div-label-content">
					...
					<label id="div-label-title">取号时间:</label><label id="div-label-content">${regRecord.retrieveTime}</label>
					<label id="div-label-title">预约时间:</label><label id="div-label-content"><fmt:formatDate value="${regRecord.registrationDate}" pattern="yyyy-MM-dd,EEEE" /></label>
				</div>
				<div id="div-record">
					<label id="div-label-title">取号地点:</label><label id="div-label-content">${regRecord.retrieveAddress}</label>
					<label id="div-label-title">操作人:</label><label id="div-label-content">${regRecord.operatorName}</label>
				</div>
				</td>
				<td>
					<div>
						<label id="div-label-title">处理状态:</label><label id="div-label-content">
						...
						<c:if test="${regRecord.isGuide=='5'}">
							<a href="javascript:process('${regRecord.registrationRecordId}')">处理</a>
						</c:if>
						<c:if test="${regRecord.isGuide=='6'}">挂号失败</c:if>
						</label>
					</div>
					<div>
						<label id="div-label-title">短信状态:</label><label id="div-label-content">
							...
							<c:if test="${today <= diagnosisDate && regRecord.unsubscribeType == '1'}">
								<a href="javascript:sendsms('${regRecord.registrationRecordId}')">重 发 </a>
							</c:if>
							<c:choose>
					...
					<div>
						<label id="div-label-title">操作:</label><label id="div-label-content">
							<c:if test="${regRecord.unsubscribeType=='1' && regRecord.isGuide==1}">
								<a href="javascript:toCancel('${regRecord.registrationRecordId}','<fmt:formatDate value="${regRecord.latestUnsubscribeTime}" pattern="yyyy-MM-dd"/>','${regRecord.doctorId}')">取消</a>
							</c:if>
							...
							<c:if test="${regRecord.unsubscribeType=='5'}">排班变更</c:if>
							<c:if test="${regRecord.unsubscribeType=='6'}">客服取消</c:if>
						</label>
					</div>
					<div>
						<label id="div-label-title">打印:</label><label id="div-label-content">
							<c:if test="${scene!='2' && scene!=6}">
								<a href="javascript:preView('${regRecord.hospName}','${regRecord.patientName}','${regRecord.phoneNum}','${regRecord.idCard}','${regRecord.departmentName}','${regRecord.doctorName}','<fmt:formatDate value='${regRecord.diagnosisDate}' pattern='yyyy-MM-dd'/>','${regRecord.expenseText}','${regRecord.retrieveAddress}','<fmt:formatDate value='${regRecord.registrationDate}' pattern='yyyy-MM-dd HH:mm:ss'/>','${regRecord.retrieveTime}','${regRecord.ampm}','<fmt:formatDate value='${regRecord.timespanStart}' pattern='HH:mm'/>','<fmt:formatDate value='${regRecord.timespanEnd}' pattern='HH:mm'/>');">打 印</a>
								<!-- <button type="button" class="btn btn-primary btn-sm" onclick="preView('${regRecord.hospName}','${regRecord.patientName}','${regRecord.phoneNum}','${regRecord.idCard}','${regRecord.departmentName}','${regRecord.doctorName}','<fmt:formatDate value='${regRecord.diagnosisDate}' pattern='yyyy-MM-dd'/>','${regRecord.expenseText}','${regRecord.retrieveAddress}','<fmt:formatDate value='${regRecord.registrationDate}' pattern='yyyy-MM-dd HH:mm:ss'/>','${regRecord.retrieveTime}','${regRecord.ampm}','<fmt:formatDate value='${regRecord.timespanStart}' pattern='HH:mm'/>','<fmt:formatDate value='${regRecord.timespanEnd}' pattern='HH:mm'/>');">打 印 </button> -->
							</c:if>
							<c:if test="${scene=='2' && scene!=6}">
