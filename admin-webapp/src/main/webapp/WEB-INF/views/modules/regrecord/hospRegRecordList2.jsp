...
<body>
	...
	<div id="recordDiv">
	<form name="form" role="form" action="${ctx}/regrecord/list" method="GET" style="height:34px">
	...
	<button type="button" class="btn" onclick="submitRed();">确 认</button>&nbsp;&nbsp;
		<button type="button" class="btn" style="float:right;"onclick="resetRadio();">重 置</button>
...
$(document).ready(function() {
	...
	var url = ctx+"/regrecord/queryProcessed";
	var sortOrder = "${sortOrder}";
...
function download(){
	var hospId = $("#hospId").val();
	var queryBeginDate1 = $("#queryDiagnosisDateBegin").val();
	var queryEndDate1 = $("#queryDiagnosisDateEnd").val();

	//alert(queryBeginDate1+"="+queryEndDate1);
	var url="${ctx}/regrecord/download?flag=2&hospId="+hospId+"&queryBeginDate="+queryBeginDate1+"&queryEndDate="+queryEndDate1;
	window.open(url);
}
...
/* function download(){
	var hospId = $("#hospId").val();
	var date = $("#date").val();
	var url="${ctx}/regrecord/download?hospId="+hospId+"&date="+date;
    window.open(url);
} */
...
function submitRed(){
	$.confirm("确定处理挂号信息吗？处理后不可恢复！", function() {
		var url = ctx+"/regrecord/saveMiss";
