	...
	<div>
		<table>
			<tr>
				<td width="230">
					<label>平台:</label> 
					<select class="mSelect" id="selectaSomeKey" onchange="selectSomething()">
							<option value="">请选择医院配置</option>
							<c:forEach items="${singleList}" var="single" varStatus="status">
								<c:if test="${someKey == single.hospitalId}">
									<option value="${single.hospitalId}" selected="selected">${single.hospName}</option>
								</c:if>
								<c:if test="${someKey != single.hospitalId}">
									<option value="${single.hospitalId}">${single.hospName}</option>
								</c:if>
							</c:forEach>
					</select>
				</td>
				<td class="text-center" width="80">
					<form id="form-list" name="form" role="form" action="${ctx}/import_quartz/single" method="POST">
