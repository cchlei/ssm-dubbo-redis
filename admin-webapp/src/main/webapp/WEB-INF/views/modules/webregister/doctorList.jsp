...
<body>
	...
	<div>
	<form name="form" role="form" action="${ctx}/webregister/doctorList" method="GET">
	...
	<input type="submit" value="查询" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <font color="#FF0000">*必须选择科室* </font>
	<div class="panel panel-default">
		...
		<table class="table table-bordered table-hover tablesorter">
			...
			<tr>
				...
				<td class="text-center"><img src="${imgPath}${doctor.doctorHeadImage}" id="img1" width="100px" height="100px"  /></td>
				...
				<c:if test="${flag==''||flag==null}">
					<c:if test="${vip!=1}">
						<c:if test="${doctor.registrationType==1}">
							<button type="button"  class="btn btn-primary" onclick="gotoResource('${doctor.doctorId}','${doctor.doctorName}','${doctor.departmentId}','${hospName}','${departmentName}');">预约 </button>
						</c:if>
	...
	<page:page pager="${page}" linkUrl="${ctx}/webregister/doctorList?flag=${params.flag}&vip=${params.vip}&areaId=${params.areaId}&belongHospId=${params.belongHospId}&hospName=${params.hospName}&departmentId=${params.departmentId}&departmentName=${params.departmentName}&hdParent=${params.hdParent}&hdChild1=${params.hdChild1}&nameSpell=${params.nameSpell}&areaIdParent=${params.areaIdParent}&areaIdChild=${params.areaIdChild}&creatorId=${params.creatorId}&creatorName=${params.creatorName}&timestamp=${params.timestamp}&token=${params.token}"/>
	</c:if>
	<c:if test="${message!=null || message!=''}">
		 ${message}
	 </c:if>	
...
function gotoResource(doctorId,doctorName,departmentId,hospName,departmentName){
	...
	if(vip=="1" && typeof(vip)!="undefined"){
		window.location.href = ctx+'/webregister/vipResource?doctorId='+doctorId+'&doctorName='+doctorName+'&hospId='+hospId+'&hospName='+hospName+'&departmentName='+departmentName+'&departmentId='+departmentId+'&vip='+vip;
	}else if(vip=="2" && typeof(vip)!="undefined"){
		window.location.href = ctx+'/webregister/resourceList?doctorId='+doctorId+'&doctorName='+doctorName+'&hospId='+hospId+'&hospName='+hospName+'&departmentName='+departmentName+'&departmentId='+departmentId+'&vip='+vip+'&creatorId='+creatorId+'&creatorName='+creatorName;
	}else if(vip=="3" && typeof(vip)!="undefined"){
		window.location.href = ctx+'/webregister/resourceList?doctorId='+doctorId+'&doctorName='+doctorName+'&hospId='+hospId+'&hospName='+hospName+'&departmentName='+departmentName+'&departmentId='+departmentId+'&vip='+vip;
...
function searchArea(areaId,areaIdChild){
	...
	var vip = $("#vip").val();
	if(vip==1){
		url = ctx + "/webregister/searchArea"; 
	}else{
		url = ctx + "/webregister/searchEnableArea";
	}
...
function searchHosp(areaId,hospId){
	...
	var vip = $("#vip").val();
	if(vip==1){
		url = ctx + "/webregister/searchHosp"; 
	}else{
		url = ctx + "/webregister/searchEnableHosp";
	}
	$.ajax({  
		type : "post",  
		url : url,  
...
function searchDep(obj,hospId1,hdParent){
	...
	var vip = $("#vip").val();
	if(vip==1){
		url = ctx + "/webregister/searchDep"; 
	}else{
		url = ctx + "/webregister/searchEnableDep";
	}
...
function searchHd(obj,number,departmentId,departmentIdChild){
	...
	var vip = $("#vip").val();
	if(vip==1){
		url = ctx + "/webregister/searchHdDep"; 
	}else{
		url = ctx + "/webregister/searchEnableHdDep";
	}
	var selPeram = "";
